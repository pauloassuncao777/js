/* player-wave.js – Wavesurfer dentro do iframe do Typebot */
(function () {
  /* --------------------------------------------------- */
  /* 1. CSS do player (injetado via <style>)             */
  /* --------------------------------------------------- */
  const css = `
  ...
  /* --------------------------------------------------- */
  /* 2. Carrega Wavesurfer, então inicia                 */
  /* --------------------------------------------------- */
  addScript('https://unpkg.com/wavesurfer.js', () => {
    console.log('[WavePlayer] Wavesurfer pronto');
    initWavePlayers();
  });

  function addScript(src, onload) {
    if (window.WaveSurfer) return onload();
  ...
  /* --------------------------------------------------- */
  /* 3. Procura e converte <audio>                       */
  /* --------------------------------------------------- */
  const profileImg = 'https://database-typebot.com/iframe/Perfil-7.png';
  const players    = [];                       // guarda todos Wavesurfer

  function initWavePlayers() {
    convertAudios();                           // 3.1 já existentes
    new MutationObserver(convertAudios)        // 3.2 os futuros
      .observe(document.body, { childList:true, subtree:true });
  }

  function convertAudios() {
    const list = document.querySelectorAll('audio[src]:not([data-wave])');
    if (list.length) console.log('[WavePlayer] novos áudios ➜', list.length);
    list.forEach(setupWavePlayer);
  }

  /* --------------------------------------------------- */
  /* 4. Cria player para um <audio>                      */
  /* --------------------------------------------------- */
  function setupWavePlayer(audioEl) {
    audioEl.dataset.wave = 'ok';               // marca como processado
    ...
    container.innerHTML = `
      ...
      <div class="microphone-icon">
        <svg viewBox="0 0 19 26">
          <path fill="#fff" d="M9.217,24.4c-1.158,0-2.1-.94-2.1-2.1v-2.366c-2.646-.848-4.652-3.146-5.061-5.958l-.053-1.404
    ...
    /* insere player e oculta áudio original */
    ...
    const btn   = container.querySelector('.play-button'),
          playI = container.querySelector('.play-icon'),
          pauseI= container.querySelector('.pause-icon'),
          mic   = container.querySelector('.microphone-icon'),
          dur   = container.querySelector('.audio-duration'),
          dot   = container.querySelector('#cursor-dot');
    let   full  = 0;

    /* play/pause + “só 1 tocando por vez” */
    btn.addEventListener('click', () => {
      if (wave.isPlaying()) { wave.pause(); return; }
      players.forEach(p => p !== wave && p.pause());
      wave.play();
    });

    /* eventos Wavesurfer */
    wave.on('ready', () => { full = wave.getDuration(); dur.textContent = fmt(full); });
    wave.on('play',  () => { playI.style.display='none'; pauseI.style.display='block'; mic.style.color='#2cacdc'; });
    wave.on('pause', () => { playI.style.display='block'; pauseI.style.display='none'; mic.style.color='#0cd464'; dur.textContent = fmt(full); });
    wave.on('finish',() => { playI.style.display='block'; pauseI.style.display='none'; mic.style.color='#0cd464'; dur.textContent = fmt(full); dot.style.left='0'; });
    wave.on('timeupdate', update); wave.on('seek', update);

    function update() {
      const cur = wave.getCurrentTime();
      dur.textContent = fmt(cur);
      const perc = cur / (full || 1);
